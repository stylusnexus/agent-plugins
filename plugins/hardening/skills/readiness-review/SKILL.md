...
```bash
COPY=$(python3 "$S/make_review_copy.py" --path <repo> [--max-file-bytes N] 2> "$RUN/copy-skipped.json")
```

`$RUN/copy-skipped.json` counts what the copy left out (credentials, binaries, files over the cap); put those counts under **Not checked**. The size cap is yours to change with `--max-file-bytes`, never the repo config's.
