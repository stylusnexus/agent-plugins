...
# Helper: update version in package.json, plugin.json, and site structured data
set_version() {
  local OLD="$1"
  local NEW="$2"
  ...
  if [ -f "$SITE_HTML" ]; then
    sed "s/\"softwareVersion\": \"$OLD\"/\"softwareVersion\": \"$NEW\"/" "$SITE_HTML" > "$SITE_HTML.tmp" && mv "$SITE_HTML.tmp" "$SITE_HTML"
    echo "Updated $SITE_HTML softwareVersion"
  fi
}
